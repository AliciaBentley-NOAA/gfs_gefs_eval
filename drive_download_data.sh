...
export SCRIPTS_PATH='/lfs/h2/emc/vpppg/noscrub/Alicia.Bentley/gfs_gefs_eval'

# Specify case name and forecast length
export CASE='testcase'
export FHR_START=0
...
export GET_RAP_ANL=false
export GET_ST4_ANL=false
export GET_NOHRSC_ANL=false
...
if [ $GET_GFS_FCSTS = true ]; then
   echo "Create/submit script to download ops/retro GFS forecasts (${CYCLE})"
...
if [ $GET_GEFS_FCSTS = true ]; then
    echo "Create/submit script to download ops/retro GEFS forecasts (${CYCLE})"
